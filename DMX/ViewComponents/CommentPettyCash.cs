...
namespace DMX.ViewComponents
{
    public class CommentPettyCash(XContext dContext, UserManager<AppUser> userManager) : ViewComponent
    {
...

        {
            ...
            pettycashToComment = (from p in dcx.PettyCash.Include(p => p.Comments.OrderBy(m => m.CreatedDate)).ThenInclude(c => c.AppUser) where p.Id.ToString() == @Encryption.Decrypt(Id) select p).FirstOrDefault();
